...
auth.post("/login", async (c) => {
  ...
  const match2 = await verifyPassword(password, result.password_hash);
  if (!match2) {
    return c.json({ error: "\u7528\u6237\u540D\u6216\u5BC6\u7801\u9519\u8BEF" }, 401);
  }
...
tenants.post("/tenants", requireAdmin, async (c) => {
  ...
  ).bind(name, building, floor, area, elevators).first();
...
tenants.put("/tenants/:id", requireAdmin, async (c) => {
  ...
  ).bind(name, building, floor, area, elevators, id).first();
...
readings.post("/readings", async (c) => {
  const body = await c.req.json();
  const { year_month, readings: readings2 } = body;
  if (!year_month || !readings2) {
  ...
  for (const el of elevators) {
    if (readings2[el] !== void 0) {
      await stmt.bind(el, year_month, readings2[el]).run();
    }
  }
  return c.json({ ok: true });
});
...
bills.post("/bills/compute", async (c) => {
  ...
  const unitPrice = setting ? parseFloat(setting.value) : 0.8;
  ...
  for (const tenant of tenants2.results) {
    ...
    details.push({
      ...
      per_sqm_sum: Math.round(perSqmSum * 1e4) / 1e4,
...
settings.put("/settings/:key", requireAdmin2, async (c) => {
