...
export default function Bills() {
  ...
  async function handleCompute() {
    ...
    setBills(null);
    try {
      const data = await api.computeBills(yearMonth);
      setBills(data.tenants);
    } catch (err) {
  ...
  return (
    <div className="page">
      ...
      {bills && bills.length > 0 && (
        <div className="bills-section" ref={printRef}>
          <div className="bill-group">
            <h3>A栋 — {yearMonth} 电梯电费</h3>
            <BillTable data={aBills} />
          </div>
          <div className="bill-group">
            <h3>B栋 — {yearMonth} 电梯电费</h3>
            <BillTable data={bBills} />
          </div>
        </div>
      )}
    ...
    <table className="table">
      ...
      <tbody>
        ...
        <tr className="total-row">
          <td colSpan="5">合计</td>
          <td className="cost">{data.reduce((s, b) => s + b.total_cost, 0).toFixed(2)}</td>
        </tr>
      </tbody>
    </table>
  );
